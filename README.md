...
- **返回**依目前工作階段的實際瀏覽紀錄移動；**上一頁／下一頁**依應用程式的固定工作頁順序
  移動，不等同瀏覽器上一頁。
- 瀏覽器重新整理會保留目前工作頁；從搜尋、建立或狀態選單前往頁面後，選單會自動收合。
...
懶人標籤生成器會在選定男／女後以空白標籤開始，方便完全手選；「全部重新隨機」則會保留
相容性規則，並避開最近幾輪已出現的高辨識度髮型、髮色、眼鏡、服裝與動作組合。獸人／半獸人
與福瑞的物種細節會在同一分類章節展開，不會因物種選擇改變導覽列數量。
